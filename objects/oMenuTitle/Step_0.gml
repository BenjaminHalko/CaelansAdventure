...
else if(phase == 2)
{
	...
	if(!instance_exists(oMenu)) and (!instance_exists(oMenuFinishedExtras)) and (!instance_exists(oMenuFinished))
	{
		if(global.completed)
		{
			var extra = false;
			...
			{
				if(Load("Extras","Collected - "+string(i),false))
				{
					extra = true;
					break;
				}
			...
			if(extra)
			instance_create_layer(64,320,"Menu",oMenuFinishedExtras);
			else
			instance_create_layer(64,320,"Menu",oMenuFinished);
		}
		else
		{
...
else if(phase == 4)
{
	...
	if(iconscale == 200)
	{
		...
		if(rm == rFinalBossCastle) or (rm == rFinalBossEvilChrisFightIntro)
		SlideTransistionFlash(TRANS_MODE.GOTO,room_width/2,room_height/2,rm);
		else
